...
def func_evaluate(scan_object,
    ...
    n_models | int | The number of models to be evaluated. If set to 10, then 10 models
                     with the highest metric value are evaluated. See below.
    metric | str | The metric to be used for picking the models to be evaluated.
    folds | int | The number of folds to be used in the evaluation.
    shuffle | bool | If the data is to be shuffled or not. Set always to False for
                     timeseries but keep in mind that you might get periodical/seasonal 
                     bias.
